...
export default function UserDropdown({ align = "start", user }: UserDropdownProps) {
    ...
    <DropdownMenu>
      <DropdownMenuTrigger className="cursor-pointer">
        <Avatar className="size-10">
          <AvatarImage src={`https://api.dicebear.com/9.x/dylan/svg?seed=${user.username}`} />
          <AvatarFallback className="font-medium">{user.username.charAt(0)}</AvatarFallback>
        </Avatar>
      </DropdownMenuTrigger>
      <DropdownMenuContent align={align} className="w-[280px] mt-2 rounded-lg">
