...
const UsageTracker: React.FC<UsageTrackerProps> = ({ user, onUpgrade }) => {
  ...
  const { aiUsageCount, aiUsageLimit, plan } = user.subscription;

  const usagePercentage = (aiUsageCount / aiUsageLimit) * 100;
  const isNearLimit = usagePercentage >= 80;
  const isAtLimit = aiUsageCount >= aiUsageLimit;

  // Trigger modal when free users hit their cap
  useEffect(() => {
  ...
  if (plan !== "free") return null; // Paid users don’t see usage tracker
          ...
          <div className="bg-white dark:bg-[#161B22] rounded-2xl shadow-2xl max-w-md w-full">
            ...
            <div className="relative bg-gradient-to-r from-orange-500 to-red-600 p-6 text-white rounded-t-2xl">
              ...
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <h2 className="text-xl font-bold">AI Limit Reached</h2>
                  <p className="text-white/80 text-sm">
                    You’ve used all {aiUsageLimit} free AI interactions today
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6">
              <div className="text-center mb-6">
                ...
                <p className="text-gray-600 dark:text-gray-400 text-sm">
                  Get unlimited AI assistance and unlock premium features
                </p>
              </div>
            ...
            <div className="bg-gray-50 dark:bg-[#0D0D0D] px-6 py-4 text-center rounded-b-2xl border-t border-gray-200 dark:border-gray-700">
              <p className="text-xs text-gray-500 dark:text-gray-400">
                Your limit resets in 24 hours. Upgrade for unlimited access.
              </p>
            </div>
          </div>
        </div>
