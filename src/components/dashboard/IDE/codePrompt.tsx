...
export default function CodePrompt() {
  ...
  return (
    <div className="max-h-full h-[calc(100vh-75px)] w-full flex overflow-hidden">
      {/* Only show SplitPane in preview mode */}
      {activeTab === "preview" ? (
        <SplitPane
          split="vertical"
          ...
          <div className={`w-full pr-5 max-h-full h-[calc(100vh-78px)] ${isFullScreen ? "pl-5" : ""}`}>
            <div className="space-y-3">
              <div className="flex items-center w-fit rounded-full bg-neutral-900 p-1">
                <button
                  className={`text-sm font-medium cursor-pointer hover:scale-95 rounded-full px-4 py-2 flex items-center ${
                    activeTab === "code" ? "bg-black" : "bg-neutral-900"
                  }`}
                ...
                <button
                  className={`text-sm font-medium cursor-pointer hover:scale-95 rounded-full px-4 py-2 flex items-center ${
                    activeTab === "preview" ? "bg-black text-blue-500" : "bg-neutral-900"
                  }`}
              ...
              <div className="w-full">
                <div className="flex justify-between w-full items-center gap-4">
                  ...
                  <Scaling
                    size={17}
                    className={`cursor-pointer hover:text-blue-500 ${isFullScreen ? "text-blue-500" : ""}`}
        ...
        /* Full width code editor when in code mode */
        <div className="w-full h-full">
          <div className="space-y-3">
            ...
            <div className="w-full overflow-hidden h-[calc(100vh-120px)]">
              <EditorIDE />
            </div>
          </div>
        </div>
