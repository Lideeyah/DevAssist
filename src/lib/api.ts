...
export class DevAssistAPI {
  ...
  private async request(method: string, endpoint: string, data: any = null, includeAuth = true): Promise<any> {
    ...
    // Ensure tokens are loaded (in case they were updated elsewhere)
    this.loadTokens();
  ...
  // async createFile(projectId: string, fileData: { filename: string; content: string; mimeType?: string }): Promise<unknown> {
  ...
  async createFile(projectId: string, fileData: { filename: string; content: string; mimeType?: string }): Promise<any> {
    ...
    try {
      ...
      console.error(`Create file failed for project ${projectId}:`, error);
      throw error;
    }
  }
